...
def check_bst(root):
    if root is None:
        return True
    return (
        all_less_than(root.left, root.value)
        and all_greater_than(root.right, root.value)
        and check_bst(root.left)
        and check_bst(root.right)
    )


def all_less_than(node, value):
    if node is None:
        return True
    return (
        node.value < value
        and all_less_than(node.left, value)
        and all_less_than(node.right, value)
    )


def all_greater_than(node, value):
    if node is None:
        return True
    return (
        node.value > value
        and all_greater_than(node.left, value)
        and all_greater_than(node.right, value)
    )
...
#         if node is None:
#             return
#         tree_graph.add_node(pos, label=node.value, depth=depth)
